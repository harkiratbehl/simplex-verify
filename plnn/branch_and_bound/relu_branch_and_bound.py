...
class ReLUDomain:
    ...
    def __init__(self, mask, lb=-float('inf'), ub=float('inf'), lb_all=None, up_all=None, parent_solution=None,
                 parent_ub_point=None, parent_depth=0, c_imp=0, c_imp_avg=0):
        self.mask = mask
        self.lower_bound = lb
        self.upper_bound = ub
...
def relu_bab(intermediate_net, bounds_net, branching_net_name, domain, decision_bound, eps=1e-4, sparsest_layer=0,
             timeout=float("inf"), batch_size=5, parent_init_flag=True,
             gurobi_specs=None, branching_threshold=0.2, anderson_bounds_net=None, writer=None, dom_hard_crit=None,
             prob_hard_crit=None, hard_batch_size=5):
    '''
    ...
    if branching_net_name is None:
        ...
        # create more lists for the fail-safe strategy. When gnn branching decision is not satisfactory, we move
        # related domain info into *_previous lists and call heuristic choices to see any improvement can be made.
        orig_ub_stacks_previous = []; orig_lb_stacks_previous = []
        orig_mask_stacks_previous = []
        parent_init_stacks_previous = None
        # we also record already computed final bounds info, so if heuristic choices give worse performance, we
    ...
    while global_ub - global_lb > eps:
        ...
        hard_task = bab.is_hard_problem(domains, prob_hard_crit, hard_task)
       ...
       ###picks not 1 problem but effective_batch_size number of problems/domains.
        for batch_idx in range(effective_batch_size):
            ...
            # TODO: adapt GNN branching to ParentInit class (and ExpLP)
        ...
        if heuristic_choice:
            ...
            if len(orig_lb_stacks_previous) != 0:
                branching_decision_list_temp, _ = branching_tools.heuristic_branching_decision(
                    orig_lb_stacks_previous, orig_ub_stacks_previous, orig_mask_stacks_previous)
                branching_decision_list += branching_decision_list_temp
            if effective_batch_size > 0:
                branching_decision_list += branching_tools.gnn_branching_decision(
                    orig_lb_stacks_current, orig_ub_stacks_current, orig_parent_sol_stacks_current,
                    orig_parent_ub_points_stacks_current, orig_gl_lb_stacks_current, orig_mask_stacks_current)
        ...
        if not heuristic_choice:
            ...
            if len(recompute_indices)!=0:
                ...
                parent_init_stacks_previous = domains[0].parent_solution.as_stack(recompute_size*2)
        ...
        for batch_idx in range(dom_lb.shape[0]):
            ...
            if dom_lb[batch_idx] == float('inf') or dom_ub[batch_idx] == float('inf') or \
                ...
                if filling_hard_batch and bab.is_difficult_domain(dom_to_add, dom_hard_crit, global_ub, decision_bound):
