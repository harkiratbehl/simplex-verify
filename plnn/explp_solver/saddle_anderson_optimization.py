...
def compute_primal_dual_gap(additional_coeffs, weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, dual_vars,
                            dual_bounds, precision=torch.float):
    ...
    for lay_idx in range(1, len(dual_vars.sum_beta)):
        _, _, _, _, atom_grad, _ = dual_vars.betak_grad_lmo(
            lay_idx, weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, precision, only_grad=True)
...
def spfw_block_k_step(lay_idx, weights, clbs, cubs, nubs, l_preacts, u_preacts, dual_vars, primal_vars, iter,
                      step_size_dict, precision=torch.float, fixed_M=False, max_iters=None):
    """
    ...
    M_condgrad, W_I_M, WIlM, W1mIuM, atom_grad, WI = dual_vars.betak_grad_lmo(
        lay_idx, weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, precision)
...
def spfw_full_step(weights, clbs, cubs, nubs, l_preacts, u_preacts, dual_vars, primal_vars, iter, step_size_dict,
                   precision=torch.float, fixed_M=False, max_iters=None):
    """
    Given list of layers, primal variables (instance of SaddlePrimalVars), pre and post activation bounds,
    ...
    dual_cd, grad_alpha, beta_atom_grad, WIs = dual_vars.dual_grad_lmo(
        weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, precision, fixed_M=c_fixed_M,
...
def primals_subgrad_initializer(weights, clbs, cubs, nubs, l_preacts, u_preacts, dual_vars, primal_class, params,
                                primal_init=None):
    """
    ...
    for outer_it in itertools.count():
        ...
        primal_vars_subg = compute_primal_subgradient(weights, primal_vars, dual_vars, clbs, cubs, nubs, l_preacts,
                                                      u_preacts, bigm_flag)
...
def compute_primal_subgradient(weights, primal_vars, dual_vars, clbs, cubs, nubs, l_preacts, u_preacts, bigm_flag,
    ...
    for lay_idx in range(len(dual_vars.sum_beta)-1, -1, -1):
        ...
        if lay_idx == 0:
            ...
            M_max, W_I_max, WIlmax, W1mIumax, _, _ = dual_vars.betak_grad_lmo(
                lay_idx, weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, precision, bigm_only=bigm_flag)
...
class SaddleDualVars(anderson_optimization.DualVars):
    ...
    def betak_grad_lmo(self, lay_idx, weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, precision,
                       only_grad=False, bigm_only=False):
        """
        Given list of layers, primal variables (instance of SaddlePrimalVars), pre and post activation bounds,
        ...
        if not bigm_only:
            ...
            masked_op, Istar_k, exp_k_grad, unfolded_zk, l_check, u_check = self.anderson_oracle(
                lay_idx, weights, clbs, cubs, nubs, primal_vars, do_intermediates=False)
        ...
        if not only_grad:
            ...
            if not bigm_only:
                shape_ex = unfolded_zk if type(lin_k) in [utils.ConvOp, utils.BatchConvOp] else zk
                atom_I = torch.where(
                    lmo_is_beta0.view_as(shape_ex).unsqueeze(3), torch.zeros_like(Istar_k),
                    torch.where(lmo_is_beta1.view_as(shape_ex).unsqueeze(3), torch.ones_like(Istar_k), Istar_k))
                ...
                beta_WIl_lmo = torch.where(lmo_is_beta1, l_preact - lin_k.get_bias(),
                    masked_op.forward(l_check, bounds_matrix_in=True, add_bias=False)) * M_atom_mask
                beta_W1mIu_lmo = torch.where(lmo_is_beta0, u_preact - lin_k.get_bias(),
                    nubs[lay_idx - 1].unsqueeze(1) - masked_op.forward(u_check, bounds_matrix_in=True)) * M_atom_mask
                WI = masked_op.WI
            else:
    ...
    def dual_grad_lmo(self, weights, clbs, cubs, nubs, l_preacts, u_preacts, primal_vars, precision, fixed_M=False,
                      store_grad=False):
        """
        Given list of layers, primal variables (instance of SaddlePrimalVars), pre and post activation bounds,
