...
from plnn.simplex_solver.simplex_optimization import simplex_projection_sort
"""
    References:
...
class SimplexLinearizedNetwork(Simp_LinearizedNetwork):
    """
    Define class providing, through the define_linear_approximation method, a linear approximation of a neural network using
    Simplex cuts.
    It inherits from Simp_LinearizedNetwork utilities.
    """

    def __init__(self, layers, mode="lp-cut", n_cuts=100, cuts_per_neuron=False, store_bounds_progress=-1, intermediate_net_weights=None, decision_boundary=0):
        """
        ...
        ## storing intermediate net for using in simplex oracle
        self.intermediate_net_weights = intermediate_net_weights
    ...
    def build_model_using_bounds(self, input_domain, intermediate_bounds, init_cut_coeffs=None, n_threads=1, simplex_conditioning=False):
        """
        ...
        x_idx_max = len(lbs) - 1
        self.define_linear_approximation(input_domain, init_cut_coeffs=init_cut_coeffs, simplex_conditioning=simplex_conditioning, int_bounds_provided=True, compute_final_layer=False, x_idx_max=x_idx_max, n_threads=n_threads)
    ...
    def define_linear_approximation(self, input_domain, init_cut_coeffs=None, simplex_conditioning=False, int_bounds_provided=False, compute_final_layer=True, n_threads=1, x_idx_max=-1):
        ...
        for layer_idx, layer in enumerate(self.layers[:-1]):
            ...
            if type(layer) is nn.ReLU:
                ...
                if not int_bounds_provided:
                    ...
                    preact_vars = self.compute_pre_activation_bounds(x_idx, previous_layer, x_km1_vars, init_cut_coeffs=init_cut_coeffs, simplex_conditioning=simplex_conditioning, optimize=False)
                self.model.update()
    ...
    def compute_pre_activation_bounds(self, x_idx, layer, x_km1_vars, init_cut_coeffs=None, simplex_conditioning=False, ub_only=False, counterexample_verification=False, single_neuron=False, optimize=True):
        """
        ...
        if type(layer) is nn.Linear:
            ...
            for neuron_idx in neurons:
                ...
                if bound_diff < self.bounds_num_tolerance:
                    ...
                    if c_lb + self.bounds_num_tolerance >= 0:
                        c_lb = max(0, c_lb)
                if not self.model_built:
                    lin_expr = self.get_layer_linear_expression(layer, x_km1_vars, neuron_idx, x_idx, init_cut_coeffs=init_cut_coeffs, simplex_conditioning=simplex_conditioning)
            ...
            ##########
            ##########
            # TILL HERE it will calculate lower and upper bounds for each neuron in last layer with big-m relaxation on all layers before it
            ##########
            ##########
            if not optimize:
                return preact_vars

            self.x_solution, self.z_solution = SimplexLinearizedNetwork.combine_primal_solutions(
            ...
            if self.mode == "lp-cut" and self.applied_cuts < self.cut_treshold:
                ...
                for neuron_idx in neurons:
                    ...
                    ########
                    # ADD CUTS and then Optimize again for the last layer's lower and upper bounds.
                    ########
                    self.c_neuron = int(self.n_opt_neurons / 2) + neuron_idx
                    ...
                    for out_col_idx in col_neurons:
                        ...
                        if not self.model_built:
                            ...
                            lin_expr = self.get_layer_linear_expression(layer, x_km1_vars,
                                                                        (out_chan_idx, out_row_idx, out_col_idx), x_idx, init_cut_coeffs=init_cut_coeffs, simplex_conditioning=simplex_conditioning)
                            x_var = self.model.addVar(lb=c_lb,
                                                      ub=c_ub,
                                                      obj=0,
                                                      vtype=grb.GRB.CONTINUOUS,
    ...
    @staticmethod
    def get_layer_linear_expression(layer, x_km1_vars, out_coordinates, x_idx, init_cut_coeffs=None, simplex_conditioning=False, bias_factor=1):
        """
        ...
        if type(layer) is nn.Linear:
            ...
            if simplex_conditioning and x_idx <= len(init_cut_coeffs):
                lin_expr = lin_expr/init_cut_coeffs[x_idx-1].item()
    ...
    def insert_tighten_cut(self, model, where, first_call=False, counterexample_verification=False):
        ...
        if where == grb.GRB.Callback.MIPNODE or where == "lp-cut":
            ...
            if where == "lp-cut" or model.cbGet(grb.GRB.Callback.MIPNODE_STATUS) == grb.GRB.Status.OPTIMAL:
                ...
                if (self.mode == "mip-exact" and self.insert_cuts) or \
                    ...
                    for layer_idx in range(self.last_relu_index+1):
                        ...
                        if type(layer) is nn.ReLU:
                            ...
                            ################
                            #FINDING THE SIMPLEX CUT FOR EACH LAYER
                            ################
                            new_rho, new_lambda = self.simplex_oracle(x_idx, previous_layer)

                            #####
                            # This actually adds the most violated constraints.
                            if self.mode == "lp-cut" and self.applied_cuts >= self.cut_treshold:
                                    break
                ...
                ######
                # OPTIMIZING WITH THE NEW CONSTRAINTS
                ######
        ...
        if self.mode == "mip-exact":
            ...
            if where == grb.GRB.Callback.MIPSOL:
                ...
                if obj < 0:
                    ...
                    # TODO: I don't think this check is necessary (we could already terminate) but let's stay on the
                    #  safe side.
    ...
    def simplex_oracle(self, x_idx, previous_layer, cut_stats=None):
        """
        This function finds the most violated cutting plane as given by the simplex cutting plane equation.
        """
        W_k = previous_layer.weight
        n_iters=50
        lin_k = self.intermediate_net_weights[x_idx-1]
        xk = self.x_solution[x_idx].to(device=lin_k.weights.device)
        unfolded_in = xk

        if type(previous_layer) is nn.Conv2d:
            ...
            if x_idx==1:
                input_shape= self.lower_bounds[x_idx-1].squeeze(0).shape
                equ_layer_linear = lin_k.equivalent_linear(torch.zeros([input_shape[0], input_shape[1], 2*input_shape[2]], device=lin_k.weights.device))
            else:
                equ_layer_linear = lin_k.equivalent_linear(self.lower_bounds[x_idx-1].squeeze(0).to(device=lin_k.weights.device))

            batch_size = unfolded_in.shape[0]
            output_len = equ_layer_linear.weights.shape[0]
            input_len = equ_layer_linear.weights.shape[1]
            lambda_wb_clamped = torch.zeros(batch_size, output_len, input_len+1, device=lin_k.weights.device)
        ...
        return max_wb_col_sum.cpu(), lambda_k_j_trans.cpu()
