...
def primal_vars_do_fw_step(lay_idx, dual_vars, anchor_vars, eta, xt, zt, xhatt, y, weights, additional_coeffs, clbs, cubs, l_preacts, u_preacts):
    """
...
class DualVars:
    ...
    def as_simplex_initialization(self, weights, clbs, cubs, l_preacts, u_preacts):
        """
        ...
        return dual_vars.alpha, dual_vars.beta_0, dual_vars.beta_1, dual_vars.fs, dual_vars.gs, dual_vars.alpha_back, dual_vars.beta_1_back, gamma_list, rho_list, lambda_list

    def as_dp_initialization(self, weights, clbs, cubs, l_preacts, u_preacts):
        """
        Given the network layers and pre-activation bounds as lists of tensors,
        compute and return the corresponding initialization of the explp (Anderson) variables from the instance of this
        class.
        ...
        beta_2 = []
        beta_2_back = []
        for idx in range(len(dual_vars.beta_1)):
            beta_2.append(torch.zeros_like(dual_vars.beta_1[idx]))
            if idx<len(dual_vars.beta_1)-1:
                beta_2_back.append(torch.zeros_like(dual_vars.beta_1_back[idx]))

        return dual_vars.alpha, dual_vars.beta_0, dual_vars.beta_1, beta_2, dual_vars.fs, dual_vars.gs, dual_vars.alpha_back, dual_vars.beta_1_back, beta_2_back
