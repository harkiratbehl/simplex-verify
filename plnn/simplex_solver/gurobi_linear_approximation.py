...
class Simp_LinearizedNetwork(NaiveNetwork):
    '''
    ...
    def build_model_using_bounds(self, input_domain, intermediate_bounds, init_cut_coeffs=None, n_threads=1, simplex_constraint=True):
        """
                ...
                for neuron_idx in range(layer.weight.size(0)):
                    if not self.model_built:
                        if x_idx==1:
                            ...
                            # don't want to condition the last layer

                            #########
                            ###### TO-DO: do conditioning with lambda as well.
                            ###########

                            if x_idx <= len(init_cut_coeffs):
                                lin_expr = lin_expr/init_cut_coeffs[x_idx-1].item()
                                coeffs = coeffs/init_cut_coeffs[x_idx-1].item()
                            ########
                        else:
                        ...
                        v = self.model.addVar(lb=self.lower_bounds[x_idx][neuron_idx], ub=self.upper_bounds[x_idx][neuron_idx], obj=0, vtype=grb.GRB.CONTINUOUS, name=f'lay{layer_idx}_{neuron_idx}')
                        # v = self.model.addVar(lb=-1,
                        #                       ub=1,
                        #                       obj=0,
                    ...
                    for out_row_idx in range(out_lbs.size(2)):
                        ...
                        for out_col_idx in range(out_lbs.size(3)):
                            ...
                            if not self.model_built:
                                ...
                                if x_idx==1:
                                    # we assume input lies in l1. so we don't the l1 to simplex conditioning
                                    ########
                                    lin_expr = copy.deepcopy(layer.bias[out_chan_idx].item())
                                    ...
                                    for ker_row_idx in range(layer.weight.shape[2]):
                                        ...
                                        for ker_col_idx in range(layer.weight.shape[3]):
                                            ...
                                            if x_idx==1:
                                                ...
                                                ## conditioning the layer so that the output lies in simplex
                                                # don't want to condition the last layer
                                                if x_idx <= len(init_cut_coeffs):
                                                    coeff = coeff/init_cut_coeffs[x_idx-1].item()
                                            else:
