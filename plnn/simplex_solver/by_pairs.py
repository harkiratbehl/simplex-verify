...
class ByPairsDecomposition:
    ...
    @staticmethod
    def get_optim_primal(weights, additional_coeffs, lower_bounds, upper_bounds, dual_vars):
        rhos = dual_vars.rhos
    ...
    def get_initial_kw_solution(weights, additional_coeffs, lower_bounds, upper_bounds):
        ...
        while lay_idx > 0:
            ...
            rho = scale.unsqueeze(1) * lbda#(output(batch_size modulo)*input shape)
            rhos.append(rho)
            lay_idx -= 1
    ...
    @staticmethod
    def get_initial_naive_solution(weights, additional_coeffs, lower_bounds, upper_bounds):
        add_coeff = next(iter(additional_coeffs.values()))
        batch_size = add_coeff.shape[:2]
        device = lower_bounds[-1].device

        rhos = []# for each layer, the shape is (output(batch_size modulo)*input shape)
        #this is from 2nd layer to nth layer. so first would be (output_of_second*input_to_second)
        for lay_idx in range(1, len(weights)):#don't consider the input layer
            lay_shape = lower_bounds[lay_idx].shape[1:]
            rhos.append(torch.zeros((*batch_size,) + lay_shape,
                                    device=device))#out_neurons*input_shape
        return DualVarSet(rhos)
    ...
    def use_external_initial_solution(self, weights, additional_coeffs, lower_bounds, upper_bounds):
        ...
        return DualVarSet(self.external_init.rhos)
...
def opt_lin_domain(layer, lin_k, lin_hat_kp1, lb_k, ub_k):
    ...
    # za_k = torch.where((lin_eq > 0), lb_k.unsqueeze(1), ub_k.unsqueeze(1))
    lin_eq_matrix = lin_eq.view(lin_eq.shape[0],lin_eq.shape[1],-1)

    (b,c) = torch.min(lin_eq_matrix, 2)
    a = F.one_hot(c, lin_eq_matrix.shape[2])
    for i in range(lin_eq_matrix.shape[0]):
        for j in range(lin_eq_matrix.shape[1]):
            if b[i][j]>0:
                a[i][j]=0
    za_k = a.view(lin_eq.shape).to(dtype=torch.float32)
