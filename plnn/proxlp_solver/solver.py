...
class SaddleLP(LinearizedNetwork):
    ...
    def build_first_conditioned_layer(l_0, u_0, layer, no_conv=False):
        '''
        Conditioned layer means that the input will be from -1 to 1 and this function will scale it such that layer 
        outputs the same thing as it would if the input were from -delta to delta.
        '''
        w_1 = layer.weight
        b_1 = layer.bias
        ...
        if isinstance(layer, nn.Linear):
            ...
            if isinstance(cond_layer, LinearOp) and (l_0.dim() > 2):
                # This is the first LinearOp,
                # We need to include the flattening
                cond_layer.flatten_from((l_0.shape[1:]))
    ...
    @staticmethod
    def proximal_optimal_step_size(additional_coeffs, diff_grad, primal_vars, cond_grad, eta):
    ...
    def compute_saddle_dual_gap(self, primal_vars, dual_vars, prox_dual_vars, weights, final_coeffs, lower_bounds, upper_bounds, eta, include_prox_terms=False):
