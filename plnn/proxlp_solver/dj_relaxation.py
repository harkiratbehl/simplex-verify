...
class DJRelaxationLP(SaddleLP):
    ...
    def initialize_from(self, external_init):
        ...
        self.external_init_dual = DualVarSet(external_init.lambdas, external_init.mus)
...
class DJPInit(ParentInit):
    ...
    def as_stack(self, stack_size):
        ...
        stacked_lambdas = [pinits[0].unsqueeze(0).repeat(((stack_size,) + (1,) * (pinits.dim() - 1)))
                           for pinits in self.duals.lambdas]
        stacked_mus = [pinits[0].unsqueeze(0).repeat(((stack_size,) + (1,) * (pinits.dim() - 1)))
                       for pinits in self.duals.mus]
        DJPInit(DualVarSet(stacked_lambdas, stacked_mus))
    ...
    def set_stack_parent_entries(self, parent_solution, batch_idx):
        ...
        for x_idx in range(len(self.duals.lambdas)):
            self.duals.lambdas[x_idx][2 * batch_idx] = parent_solution.duals.lambdas[x_idx].clone()
            self.duals.lambdas[x_idx][2 * batch_idx + 1] = parent_solution.duals.lambdas[x_idx].clone()
            self.duals.mus[x_idx][2 * batch_idx] = parent_solution.duals.mus[x_idx].clone()
            self.duals.mus[x_idx][2 * batch_idx + 1] = parent_solution.duals.mus[x_idx].clone()
    ...
    def get_stack_entry(self, batch_idx):
        ...
        entry_init_dual = DualVarSet(
            [csol[batch_idx].unsqueeze(0) for csol in self.duals.lambdas],
            [csol[batch_idx].unsqueeze(0) for csol in self.duals.mus]
        )
    ...
    def get_lb_init_only(self):
        ...
        lb_init_dual = DualVarSet(
            [c_init[:, -1].unsqueeze(1) for c_init in self.duals.lambdas],
            [c_init[:, -1].unsqueeze(1) for c_init in self.duals.mus]
        )
