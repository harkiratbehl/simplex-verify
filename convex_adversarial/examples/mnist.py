...
def select_model(m): 
    if m == 'large': 
        ...
        model = pblm.mnist_2layer().cuda()
        print(model)
        input('correct place')
    elif m == 'mnist_3layer': 
        model = pblm.mnist_3layer().cuda()
        print(model)
        input('correct place')
    elif m == '500': 
        model = pblm.mnist_500().cuda()
    elif m == '100': 
        model = pblm.mnist_100().cuda()
    elif m == '20': 
        model = pblm.mnist_20().cuda()
    elif m == 'mnist_conv_2':
        model = pblm.mnist_conv_2().cuda()
    elif m == 'mnist_conv_1':
        model = pblm.mnist_conv_1().cuda()
    elif m == 'mnist_conv_2_lin':
        model = pblm.mnist_conv_2_lin().cuda()
    elif m == 'mnist_conv_2_conv':
        model = pblm.mnist_conv_2_conv().cuda()
    else:
        input('wrong place')
